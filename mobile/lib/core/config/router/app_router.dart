...
import 'package:mobile/features/pagos/presentation/screens/pagos_list_screen.dart';
import '../../../features/pagos/presentation/screens/pago_screen.dart';
...
final GoRouter appRouter = GoRouter(
  ...
  routes: [
    ...
    GoRoute(
      path: '/pago/:incidenteId',
      builder: (_, state) => PagoScreen(
        incidenteId: int.parse(state.pathParameters['incidenteId']!),
        descripcionIncidente: state.uri.queryParameters['desc'] ?? 'Servicio vehicular',
      ),
    ),
    
    GoRoute(path: '/pagos', builder: (_, _) => const PagosListScreen()),
    GoRoute(path: '/vehiculos', builder: (_, _) => const VehiculosScreen()),
    GoRoute(path: '/vehiculos/agregar', builder: (_, _) => const AgregarVehiculoScreen()),
    GoRoute(path: '/notificaciones', builder: (_, _) => const NotificacionesScreen()),
  ],
);